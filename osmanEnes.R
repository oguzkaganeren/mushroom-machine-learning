...
### mtry value continue as 1 after 6. value.
...
############################# for ntree
## k fold cross validation takes too much time. So that, We optimize the 1-20 ntree.
start_time <- Sys.time()
ntreeAccuracy <- list()
for(i in c(1:20)){
  set.seed(579642)
...
rf_mtry <- train(bruises~.,
                 ...
                 tuneGrid = tuneGrid,
                 trControl = trControl,
                 do.trace = TRUE, ## is given about oob of randomForest.
                 ntree = ntreeMax)
print(rf_mtry)
plot(rf_mtry, type="o" , bty="l" , ylab="Accuracy" , xlab="mtry Values" , col=rgb(0.1,0.5,0.1,0.8) , lwd=0.5 , pch=16  )
rf_mtry$results$Accuracy
